...
- `Personal` — kept off the division board and out of the Command Centre's ministry-wide counters. Visible to its owner, its creator, anyone added as a collaborator, plus: **Super Admin and OSD** (every personal task, ministry-wide, by role) and **any user carrying the `can_see_personal_tasks` grant** (the personal tasks of every division they are a member of or head). Super Admin sets that grant per user — the "Personal task visibility" toggle on Users → Create / Edit — and it is audited as a `role_change`. It is seeded on for Directors, Deputy Secretaries, Under Secretaries and sitting division heads. PMU members and JS are excluded structurally: their branches return before the grant is considered, so PMU isolation and the JS surface hold even if the flag is set. Enforced in `buildVisibilityClausesFrom` (`src/lib/visibility-rules.ts`). *(Changed 2026-09-07; personal was previously invisible to superiors.)*
...
- **Opens grouped by default** for leadership and division heads; a multi-division member may group but lands flat. Everyone else gets the flat three-segment list (Tasks assigned to me / Personal / Other tasks of my division), and `?group=none` returns anyone to it. *(Heads added 2026-09-17 — they already curated lanes, edited the Notice board and generated the report, but the flat list gave those rights nowhere to appear.)*
- **PMU members keep the flat list but get the lane board inside each segment** — a PMU is one team, not a set of divisions to group, so the Daily / Weekly / FortNight / Monthly / Watchlist board (plus the Show-task-cards toggle) renders within "Tasks assigned to me", "Personal tasks" and "Other tasks of my PMU team" instead of in a division card. Read-only pills: `canSetJsPriorityLane` grants a PMU member no curation, though a PMU team leader still edits Latest status on their team's tasks via `canManageTask`. *(Added 2026-09-17.)*
