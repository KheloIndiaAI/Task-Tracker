...
- **Users form by organization.** Super Admin → Users (and Structure's Edit / Add member) asks for the **Organization** first, then offers only its divisions, grouped by directorate ("Directly under SAI", "RC Bengaluru"). The Organization picker is not stored — a person's organization is always the one their home division sits in. **Additional divisions** lists every organization as its own collapsible block, sub-grouped by directorate, with the home division shown as such. One pure grouping (`groupDivisionsByOrganization` in `structure-shared.ts`) feeds all three, so they cannot disagree.
