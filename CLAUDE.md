...
- Organization hierarchy — `organization` and `directorate` levels above `division` in the one `divisions` tree, the ministry's structure wrapped in **Ministry Headquarter**, the Organization-head toggle, and head powers cascading down — done (see "Organizations and directorates" under the permission model below)
- Organization → Directorate → Division pickers, and SAI grades — Super Admin → Users picks an **Organization**, then (where it has any) a **Directorate**, then a **Division** within it; **Quick Create** asks the same three for the task's board, over only the boards the person may create in, prefilled from their placement; **Additional divisions** are grouped by organization and directorate, so same-named divisions (two "NCOE"s) are told apart. New **Regional Director** and **Assistant Director** slots — done (see "Organizations and directorates" below)
- Status access and Task scheduling access — two Super-Admin-managed grants on Users, so a Super Admin can let chosen people write the **Status** line on the tasks list and put tasks (including unscheduled ones) into the **Daily / Weekly / FortNight / Monthly / Watchlist** lanes — done (see the task-fields notes below)
...
**Extended 2026-09-18:** the lane slider's arrows on the tasks list (`SlideArrow` in `DivisionLaneBoard.tsx`) are amber — `--accent-soft` fill, `--accent-line` border, `--accent` chevron. Chrome, not a priority signal, so the rule would say neutral; but the only thing those arrows do is page between the JS Priority lanes, and in neutral grey beside a long column they were genuinely hard to find. Scoped to that one control on that one board.
...
- **One deliberate, per-task hole in that isolation**: the **"Show this task to PMU team"** switch (`tasks.shared_with_pmu_team`, rule `canSharePmuTeam`) on a division task shows that one task down to the PMU team(s) under its division, who can then collaborate on it like any other participant. It is a **head power** (the division's head or delegate, plus OSD / Super Admin) — ownership alone is not enough — and the switch only appears when the division actually has a PMU under it. The same flag on a PMU's OWN task keeps its original meaning: the team leader sharing it with their whole team — and there it is **on by default at creation** (`resolvePmuTeamShareOnCreate` in `src/lib/pmu-team-shared.ts`), since work put on a PMU's board is the team's work. A division task stays opt-in. Settable in Quick Create and on the task's Collaborators section; cleared automatically if the task moves division. *(Division direction added 2026-09-17; PMU default-on 2026-09-17.)*
- **A division can carry several PMUs, and each has its own Team Head.** Khelo India Scheme has INFRA_PMU, KIS_PMU and PPP_PMU. Super Admin → Structure & hierarchy shows one "team head" card **per PMU** — on the parent division (all of its PMUs) and on the PMU node itself. `setPmuTeamHeadAction` takes the **PMU id**, and promote/clear are scoped to that PMU's `pmu_id`, so sibling PMUs are never touched. *(Per-PMU heads 2026-09-17; it previously took the parent division id, showed whichever leader sorted first as "the" head, pooled candidates across every PMU, and cleared demoted all of them at once.)*
...
The division board is the **only** task-list design on `/tasks`. Every user lands on it: a card per division carrying the Notice board, the sub-division / PMU filter pills, the Daily / Weekly / FortNight / Monthly / Watchlist lane board, and the per-division completed list.

- **No per-role gate.** Each control inside the card is already permissioned per user — `canEditDivisionNotice` for the notice, `canSetJsPriorityLane` for the lane pills, `canManageTask` / contributor rights for Latest status, `can_add_js_comment` for JS Comment — so a reader who may change none of them still gets a far better read of their division's work than a wall of cards gave them. Read-only is the normal case, not a degraded one.
- **`?group=none`** is the explicit opt-out, rendering the flat list (Tasks assigned to me / Other tasks of my division) as plain cards with no lane board. `resolveGroupByDivision` in `src/lib/task-grouping-shared.ts` still resolves the tri-state param; the page simply passes `true` as the default for everyone.
- **There is no "Tasks assigned to me" section on the default view** — the trade-off of one design. Ownership shows on each card's avatar, and `?filter=mine` still narrows the whole list to your own work.
- *(History, all 2026-09-17: the board started as leadership-only, gained division heads, then PMU members via a separate per-segment layout, and finally dropped the gate entirely — one design for everyone. The per-role rules `canGroupTasksByDivision` / `opensTasksGrouped` and the PMU segment-board branch were deleted with it.)*
...
- **Latest status** (`tasks.latest_status`) — capped at 50 words, shown above Context on the task detail page too (`SectionLatestStatus`). Same contribute right as Context: owner, creator, an explicit collaborator, or anyone @mentioned in the discussion (`isTaskContributor` / the batched `getContributorTaskIds`), plus anyone who can otherwise manage the task (`canManageTask`), plus anyone carrying the Super-Admin-managed **Status access** grant (`users.can_edit_latest_status`, the "Status access" toggle on Users → Create / Edit, audited as a `role_change`). That grant carries no division scope: it reaches any task its holder can already **see**, and opens the Status line and nothing else — `canEditLatestStatus` in `rbac/rules.ts` for the surfaces, re-checked in `updateTaskFieldsAction` against `isTaskVisibleTo`, which rejects any other field on the same save. One save path backs both surfaces, so they can never disagree.
...
Task scheduling — the **Daily / Weekly / FortNight / Monthly / Watchlist** lanes (`tasks.js_priority_lane`):
- Set by Super Admin, OSD, a Director-grade member of the task's division, or its head — plus anyone carrying the Super-Admin-managed **Task scheduling access** grant (`users.can_schedule_tasks`, the toggle on Users → Create / Edit, audited as a `role_change`). Like Status access it has no division scope of its own: it reaches any task its holder can already **see**, including one **not scheduled yet** — `canSetJsPriorityLane`, re-checked in `updateTaskJsPriorityAction` against `isTaskVisibleTo`.
- One rule across every surface: the lane board's pills and clock, the task page's lane picker, and the mobile card's "Add to Priority Board" action. *(The task page previously offered the picker to Super Admin and OSD alone, narrower than the board and the server action; 2026-09-23 aligned it.)* Ordering the JS Priority Board itself by drag-and-drop stays OSD / Super Admin.
...
Organizations and directorates (added 2026-09-21):
- The `divisions` tree reads, top down: **organization** → optional **directorate** → **division** → sub-division → section, with PMU teams attached to divisions. `DivisionKind` gained `organization` and `directorate`; the pure rules — labels, the parent matrix (`allowedParentKinds`), `organizationOf`, and the cascade (`expandHeadedToDescendants`) — live in `src/lib/structure-shared.ts`, covered by `tests/structure-shared.test.ts`.
- **Ministry Headquarter** is the organization holding the ministry's original structure. Migration `20260921120200_create_ministry_headquarter` created it and made it the parent of every division that had none — nothing else moved: no task, user, PMU, sub-division, section, notice board, headship or delegation changed. Each **Regional Centre** is another organization; its Assistant Directors head **directorates**, with the KIC / STC / NCOE / KISCE divisions under them. Regional Directors and Assistant Directors have their own slots, `regional_director` and `assistant_director` (added 2026-09-22): by product decision each carries **exactly the Director's slot powers** in the divisions they are a member of — every rule asks `isDirectorGrade` / `DIRECTOR_GRADE_SLOTS` (`src/lib/hierarchy-slots.ts`) instead of testing `'director'`, so the three cannot drift apart. Rank: Regional Director at level 3 with Director, Assistant Director at level 5 with Under Secretary (`HIERARCHY_SLOT_LEVEL`).
- **Organization → Directorate → Division pickers.** Super Admin → Users (and Structure's Edit / Add member) asks for the **Organization**, then — only where it has directorates — the **Directorate** (or "Directly under <organization>"), then the **Division** there; the ministry has no directorates, so it goes straight from Organization to Division. **Quick Create** asks the same three for the task's board (Division or PMU), built only from the boards the person may create in (`createTargets`, placed by `groupByPlacement` in the app layout — a PMU sits with its parent division): a step with a single choice stays on screen, locked, so the person always sees where the task will land, and the pickers open on their own placement (home division; a PMU member's PMU). None of these pickers is stored — a person's organization is always their home division's, and the task still submits only its board. **Additional divisions** lists every organization as its own collapsible block, sub-grouped by directorate, with the home division shown as such. One pure grouping (`groupByPlacement`, and `groupDivisionsByOrganization` over it, in `structure-shared.ts`) feeds them all, so they cannot disagree.
- **A division's parent is required** — an organization or a directorate. An organization has no parent; a directorate's parent is an organization (`createDivisionAction`).
- **Only divisions and PMU teams carry tasks** (`isTaskBoardKind`): task create and the division move refuse any other kind, and Timeline Files are marked to divisions only. People are homed in divisions, never directly in an organization or directorate (`resolveUnitPlacement`), so Structure & hierarchy shows what an organization / directorate holds instead of an officer chart.
- **Heads, cascading down.** Every **Super Admin** heads every organization by role. The **Organization head** toggle (Super Admin → Users, `users.is_organization_head`, audited as a `role_change`) makes a user head of the organization their **home division** sits in. A **directorate** has one head (`head_user_id`, set on its head card like a division's). Heading an organization or directorate is **full head power over every division beneath it** — exactly what a division head has — resolved in one place, `getHeadedDivisionIds` / `getHeadedDivisionsByUser`, so every head check inherits it. Each division's own head keeps theirs; PMUs are reached only through those divisions' existing head rules. An organization or directorate head can **delegate** it from their profile like a division head, and the delegate holds the whole cascade for the window.
- **Task pickers stay flat by division, scoped to the signed-in user**: the report dialog and the search filter list the divisions holding tasks that user can read (`fetchTaskDivisionOptions`); Super Admin / OSD get every division.
