        ...
        <div className="flex flex-col gap-4">
          <Field label="Division">
            {divisions.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                <label className="flex items-center gap-2 text-[12.5px] font-medium text-ink cursor-pointer pb-2 border-b border-line-2">
                  <input
                    type="checkbox"
                    checked={divisionIds.size === 0}
                    onChange={() => setDivisionIds(new Set())}
          ...
          <Field label="Priority">
            <div className="flex flex-wrap gap-1.5">
              <PriorityPill
                label="All"
                active={priorities.size === 0}
                onClick={() => setPriorities(new Set())}
              />
              {REPORT_CADENCES.map((c) => (
                <PriorityPill
                  key={c}
                  label={REPORT_CADENCE_LABEL[c]}
                  active={priorities.has(c)}
