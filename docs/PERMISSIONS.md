...
> **2026-09-21 — organizations and directorates.** Two levels now sit above divisions in the one `divisions` tree: **organization** (the root — "Ministry Headquarter" holds the original ministry structure, each Regional Centre is another) and an optional **directorate** (headed by an Assistant Director). Heading either is full head power over every division beneath it; Super Admins head every organization. See §5.20. Wherever this document says "division head", an organization or directorate head over that division has the same power.
...
| Regional Director | `regional_director` | Director grade (2026-09-22): exactly the Director's slot powers. Rank 3, with Director |
| Assistant Director | `assistant_director` | Director grade (2026-09-22): exactly the Director's slot powers. Rank 5, with Under Secretary |
...
- **Division task shown to the PMU team**: a division's head (direct or an active delegate; OSD / Super Admin too) can flip **"Show this task to PMU team"** on a task of their own division, setting the same `tasks.shared_with_pmu_team = true`. The scoper then adds `{ visibility: 'division', shared_with_pmu_team: true, divisionId: <the caller's PMU's parent division> }` for every PMU member, so they see that one task and nothing else from the division's board — the single, per-task hole in PMU isolation, opened only by the person who runs that board. Those members are already **participants** of the parent division (their home division is it, per the Users form's "PMU must belong to the chosen division" rule), so they can comment, be added as collaborators, be assigned subtasks and be @mentioned on it with no further grant. The switch is shown only when the division actually has a PMU under it, and refused server-side otherwise (`canSharePmuTeam`). Settable at creation (Quick Create) and any time after, from the task's Collaborators section. Ownership alone is NOT enough — widening who sees the board is a head power.
...
### 5.19 PMU Team Head admin scope

Each PMU inside a division has one **PMU Team Head** — the PMU member with `users.pmu_role = 'pmu_team_leader'` (active). A **Super Admin** designates the head directly on the **Structure & Hierarchy** page (`setPmuTeamHeadAction` + the `PmuTeamHeadCard`). **One card per PMU**, shown on the parent division (one for each PMU under it) and on the PMU node itself; the card lists the active members of **that PMU** (`users.pmu_id`), and promoting one demotes any previous head of that same PMU to `pmu_senior_consultant`, so a PMU has at most one head and sibling PMUs are untouched. *(Per-PMU from 2026-09-17: the action previously keyed on the parent division, which conflated the heads of a division's several PMUs — see CLAUDE.md.)* The head administers the **PMU team only** — the team being every active PMU member sharing the head's division and PMU attachment (`pmu_id`, matched as-is so the common "flagged member in a normal division, `pmu_id = null`" case groups by division). `getPmuTeamMemberIds(userId)` (`src/lib/pmu-team.ts`) resolves the team, returning `[]` for anyone who is not an active team head, so the scope is inert for every other user.

- **Read.** The head additionally sees their team's **division-visibility** tasks. `buildVisibilityClausesFrom` (`src/lib/visibility-rules.ts`) emits an owner-scoped clause `{ visibility: 'division', ownerId: { in: <team ids> } }` from `opts.pmuTeamLeaderMemberIds`, populated by `buildVisibilityClauses`. A teammate's **personal** task stays private (the clause is `division`-scoped).
- **Manage.** Over a **division** task **owned by a team member**, the head may edit status / priority / context, manage subtasks, and add / remove collaborators (`canManageTask`'s `pmuTeamMemberIds` branch, gated on `visibility === 'division'`), attach / rename / delete documents (`canEditTaskAttachments`), and **allot / reassign** the owner — freely to another team member, otherwise via the normal transfer matrix (`reassignTaskAction`).
- **Delete — the team's OWN tasks only.** The head may **delete** a division task (and its subtasks) owned by a team member, via `canPmuHeadDeleteOwnedTask` (`deleteTaskAction`) — **but only when the task was NOT allotted (created) by an elevated role** (a Division Head, Super Admin, or OSD; `isElevatedOverDivision`). A task those roles hand the team stays under their exclusive delete control. Subtask delete keys on the **parent** task (not the subtask's assignee).
- **Documents — never an elevated role's upload.** The head may delete the team's documents, but **never a document uploaded by a Division Head / Super Admin / OSD** (`deleteAttachmentAction`, gated by `hasFullTaskAttachmentRights` + `isElevatedOverDivision`). The uploader themselves and full-rights holders (Super Admin / OSD / owner / creator) are unaffected.
- **Scope limits — the head never gains:** any power over the division's **non-PMU ministry tasks** (owner not on the team) or another PMU / division; **redefine** (name / due date / recurrence — `canEditTaskDetails`); **visibility / division** change; or delete of a task/document controlled by an elevated role. This is management of the team's board tasks, never division-head power.
- The grant flows through the shared `canManageTask` / `canEditTask` gates plus the dedicated delete helpers, so every server action honours it consistently, and the task-detail / task-list UIs mirror it exactly.

### 5.20 Organizations and directorates (2026-09-21)

The organization tree, top down: **organization** → optional **directorate** → **division** → sub-division → section, with PMU teams attached to divisions (`pmu_parent_division_id`). All of it is the one `divisions` table, told apart by `kind`; the pure rules live in `src/lib/structure-shared.ts`.

- **Parents.** An organization has no parent. A directorate's parent is an organization. A **division's parent is required** — an organization or a directorate (`allowedParentKinds`, enforced by `createDivisionAction`). Sub-divisions, sections and PMUs are unchanged.
- **Ministry Headquarter.** Migration `20260921120200_create_ministry_headquarter` inserted the organization and set it as the parent of every division that had none. Nothing else was written: tasks, users, PMUs, sub-divisions, sections, notice boards, headships and delegations are exactly as they were, so no one's visibility or powers changed.
- **What lives where.** Only **divisions and PMU teams** carry tasks (`isTaskBoardKind`) — task create, Quick Create's targets, and the division move all refuse any other kind. Timeline Files are marked to **divisions** only. People are homed in divisions; dropping someone on an organization or directorate is refused (`resolveUnitPlacement`).
- **Who heads an organization.** Every **Super Admin**, by role. Plus any user whose **Organization head** toggle is on (Super Admin → Users; `users.is_organization_head`; its own `role_change` audit entry) — they head the organization their **home division** sits in (`organizationOf`). Moving their home division into another organization moves the headship with it.
- **Who heads a directorate.** One user, `divisions.head_user_id`, set by a Super Admin on the directorate's head card (`setDivisionHeadAction`, which refuses organizations — those are set from Users).
- **The cascade.** Heading an organization or a directorate is **full head power over every division beneath it** — visibility, task management, delete, notice board, JS-lane scheduling, report access — because `getHeadedDivisionIds` / `getHeadedDivisionsByUser` return those divisions (`expandHeadedToDescendants`) and every head check reads that list. It adds **divisions only**: PMUs are reached through the same division-head rules that already fold a division's PMUs in, and sub-divisions / sections carry no powers of their own. A plain division head's list is exactly what it was.
- **Delegation.** An organization or directorate head can delegate it from their profile, exactly as a division head delegates a division (same eligible delegates, same window rules, Super Admin notified). The delegate holds the whole cascade for the window; a delegate cannot re-delegate.
- **Pickers.** Division pickers stay **flat by division** and are scoped to the signed-in user: the report dialog and the search filter offer the divisions holding tasks the caller can read (`fetchTaskDivisionOptions`); Super Admin / OSD are offered every division. Organizations and directorates never appear in a task picker.

### 5.21 Director grade — Regional Director and Assistant Director (2026-09-22)

Two hierarchy slots for Sports Authority of India officers, `regional_director` and `assistant_director`. By product decision both are **Director grade**: every rule this document grants to the Director slot applies to them identically, in the divisions they are a member of (home or admin-granted extra) — managing and redefining every task there (`canManageTask`, `canEditTaskDetails`), setting the Daily / Weekly / Fortnight / Monthly / Watchlist lanes (`canSetJsPriorityLane`), changing a Timeline File's status, priority and action documents, and receiving the Director notifications (a file marked to their division, a file falling due, a task put on a lane). The code never tests `'director'` alone: it asks `isDirectorGrade` or queries `DIRECTOR_GRADE_SLOTS` (`src/lib/hierarchy-slots.ts`).

The slot is rank and these powers only. Heading a Regional Centre (a directorate) or a division is separate and adds the head powers of §5.20. Rank for display: Regional Director at level 3 with Director, Assistant Director at level 5 with Under Secretary.

### 5.22 Status access and Task scheduling access (2026-09-23)

Two Super-Admin-managed grants on **Users → Create / Edit**, both off by default and each audited as its own `role_change`. Neither carries a division scope: each reaches exactly the tasks its holder can already **see**, and nothing wider.

- **Status access** (`users.can_edit_latest_status`) — write the **Status** line (`tasks.latest_status`) shown under each task on the Daily/Weekly/Fortnight/Monthly board, and as "Latest status" on the task page. The owner, creator, collaborators, @mentioned users and anyone who can manage the task already may (§5.15); this widens who else can. It is that one field: `updateTaskFieldsAction` rejects any other field submitted with it, and confirms the task is visible to the holder (`isTaskVisibleTo`). Rule: `canEditLatestStatus`.
- **Task scheduling access** (`users.can_schedule_tasks`) — put a task into the **Daily / Weekly / FortNight / Monthly / Watchlist** lanes (`tasks.js_priority_lane`), including a task that is **not scheduled yet**, and move or clear it. Super Admin, OSD, a Director-grade member of the task's division and its head already may; this widens who else can. `updateTaskJsPriorityAction` confirms visibility the same way. Rule: `canSetJsPriorityLane`.

Both rules are shared by every surface that offers the control — the tasks list, the task page, and the mobile card actions — so the UI never offers what the server would refuse, nor hides what it would allow. Ordering the JS Priority Board by drag-and-drop remains OSD / Super Admin and is untouched by either grant.
